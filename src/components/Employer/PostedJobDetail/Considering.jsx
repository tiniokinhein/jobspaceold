...
const Considering = ({overview}) => {
    ...
    const {considerings} = useSelector((state) => state.recruitment)
    const [checkedApplicants, setCheckedApplicants] = useState([]);
    const [needToUpdate, setNeedToUpdate] = useState(false);
    const fetchConsidering = useCallback((limit, pageCnt, filter, search, needToUpdate) => {
        const params = helper.postJobParams(limit, pageCnt, 6, filter, search);
        dispatch(recruitmentActions.getConsiderings({uuid: postedJobId, params: params})).then(res => {
    ...
    useEffect(() => {
        ...
        fetchConsidering(limit, pageCnt, filter, search, needToUpdate)
        dispatch(setProgress(100));
        setLoading(false);
        // eslint-disable-next-line
    }, [fetchConsidering, pageCnt, search, filter, needToUpdate]);
    ...
    const handleParentCheck = (event) => {
        if (event.target.checked) {
            setCheckedApplicants(helper.pluck(considerings, 'user_id'))
        } else {
            setCheckedApplicants([])
        }
    }
                    ...
                    <Grid item xs={12} md={8.3}>
                        {!cvLoading && !loading ?
                            (Object.keys(applicantInfo).length > 0 ?
                                ...
                                <NoData title="considering data"/>) : null
                        }
                    </Grid>
                </Grid>
            </Box>
