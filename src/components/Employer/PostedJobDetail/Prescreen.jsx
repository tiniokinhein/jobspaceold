...
const Prescreen = ({overview}) => {
    ...
    const {prescreens} = useSelector((state) => state.recruitment)
    const [checkedApplicants, setCheckedApplicants] = useState([]);
    const [needToUpdate, setNeedToUpdate] = useState(false);
    const fetchPrescreens = useCallback((limit, pageCnt, filter, search, needToUpdate) => {
        const params = helper.postJobParams(limit, pageCnt, 1, filter, search);
        dispatch(recruitmentActions.getPrescreens({uuid: postedJobId, params: params})).then(res => {
    ...
    useEffect(() => {
        ...
        fetchPrescreens(limit, pageCnt, filter, search, needToUpdate)
        dispatch(setProgress(100));
        setLoading(false);
        // eslint-disable-next-line
    }, [fetchPrescreens, pageCnt, search, filter, needToUpdate]);
    ...
    const handleParentCheck = (event) => {
        if (event.target.checked) {
            setCheckedApplicants(helper.pluck(prescreens, 'user_id'))
        } else {
            setCheckedApplicants([])
        }
    }
                    ...
                    <Grid item xs={12} md={8.3}>
                        {!cvLoading && !loading ?
                            (Object.keys(applicantInfo).length > 0 ?
                                ...
                                <NoData title="pre-screen data"/>) : null
                        }
                    </Grid>
                </Grid>
            </Box>
