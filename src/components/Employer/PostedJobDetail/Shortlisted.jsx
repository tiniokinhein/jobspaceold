...
const Shortlisted = ({overview}) => {
    ...
    const {shortlisted} = useSelector((state) => state.recruitment)
    const [checkedApplicants, setCheckedApplicants] = useState([]);
    const [needToUpdate, setNeedToUpdate] = useState(false);
    const fetchShortlisted = useCallback((limit, pageCnt, filter, search, needToUpdate) => {
        const params = helper.postJobParams(limit, pageCnt, 3, filter, search);
        dispatch(recruitmentActions.getShortlisted({uuid: postedJobId, params: params})).then(res => {
    ...
    useEffect(() => {
        ...
        fetchShortlisted(limit, pageCnt, filter, search, needToUpdate)
        dispatch(setProgress(100));
        setLoading(false);
        // eslint-disable-next-line
    }, [fetchShortlisted, pageCnt, search, filter, needToUpdate]);
    ...
    const handleParentCheck = (event) => {
        if (event.target.checked) {
            setCheckedApplicants(helper.pluck(shortlisted, 'user_id'))
        } else {
            setCheckedApplicants([])
        }
    }
                    ...
                    <Grid item xs={12} md={8.3}>
                        {!cvLoading && !loading ?
                            (Object.keys(applicantInfo).length > 0 ?
                                ...
                                <NoData title="shortlisted data"/>) : null
                        }
                    </Grid>
                </Grid>
            </Box>
