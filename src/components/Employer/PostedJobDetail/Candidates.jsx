...
const Candidates = ({overview}) => {
    ...
    const fetchCandidates = useCallback((limit, pageCnt, filter, search, needToUpdate) => {
        const params = helper.postJobParams(limit, pageCnt, 7, filter, search);
        dispatch(recruitmentActions.getCandidates({uuid: postedJobId, params: params})).then(res => {
    ...
    useEffect(() => {
        ...
        fetchCandidates(limit, pageCnt, filter, search, needToUpdate);
        dispatch(setProgress(100));
        setLoading(false);
        // eslint-disable-next-line
    }, [fetchCandidates, pageCnt, search, filter, needToUpdate]);
    ...
    const handleParentCheck = (event) => {
        if (event.target.checked) {
            setCheckedApplicants(helper.pluck(candidates, 'user_id'))
        } else {
            setCheckedApplicants([])
        }
    }
