...
const NotSuitable = ({overview}) => {
    ...
    const {notSuitable} = useSelector((state) => state.recruitment)
    const [checkedApplicants, setCheckedApplicants] = useState([]);
    const [needToUpdate, setNeedToUpdate] = useState(false);
    const fetchNotSuitable = useCallback((limit, pageCnt, filter, search, needToUpdate) => {
        const params = helper.postJobParams(limit, pageCnt, 5, filter, search);
        dispatch(recruitmentActions.getNotSuitable({uuid: postedJobId, params: params})).then(res => {
    ...
    useEffect(() => {
        ...
        fetchNotSuitable(limit, pageCnt, filter, search, needToUpdate)
        dispatch(setProgress(100));
        setLoading(false);
        // eslint-disable-next-line
    }, [fetchNotSuitable, pageCnt, search, filter, needToUpdate]);
    ...
    const handleParentCheck = (event) => {
        if (event.target.checked) {
            setCheckedApplicants(helper.pluck(notSuitable, 'user_id'))
        } else {
            setCheckedApplicants([])
        }
    }
                    ...
                    <Grid item xs={12} md={8.3}>
                        {!cvLoading && !loading ?
                            (Object.keys(applicantInfo).length > 0 ?
                                ...
                                <NoData title="not suitable data"/>) : null
                        }
                    </Grid>
                </Grid>
            </Box>
