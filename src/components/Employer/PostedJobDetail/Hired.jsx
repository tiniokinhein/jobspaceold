...
const Interview = ({overview}) => {
    ...
    const {hires} = useSelector((state) => state.recruitment)
    const [checkedApplicants, setCheckedApplicants] = useState([]);
    const [needToUpdate, setNeedToUpdate] = useState(false);
    const fetchHires = useCallback((limit, pageCnt, filter, search, needToUpdate) => {
        const params = helper.postJobParams(limit, pageCnt, 4, filter, search);
        dispatch(recruitmentActions.getHires({uuid: postedJobId, params: params})).then(res => {
    ...
    useEffect(() => {
        ...
        fetchHires(limit, pageCnt, filter, search, needToUpdate)
        dispatch(setProgress(100));
        setLoading(false);
        // eslint-disable-next-line
    }, [fetchHires, pageCnt, search, filter, needToUpdate]);
    ...
    const handleParentCheck = (event) => {
        if (event.target.checked) {
            setCheckedApplicants(helper.pluck(hires, 'user_id'))
        } else {
            setCheckedApplicants([])
        }
    }
