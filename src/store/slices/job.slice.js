...
export const jobsActions = {...slice.actions, ...extraActions};
...
function createExtraActions() {
    ...
    function promote() {
        return createAsyncThunk(
            `${name}/promote`, async ({uuid, data}, ThunkAPI) => {
                const res = await fetchWrapper.put(`/employer/job-posts/${uuid}/promote`, data)
                ThunkAPI.dispatch(setMessage('Your application promoted.'))
                ThunkAPI.dispatch(setOpen(true));
                return res.data ?? {};
            }
        )
