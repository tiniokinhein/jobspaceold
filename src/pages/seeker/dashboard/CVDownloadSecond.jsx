...
const CVDownloadSecond = () => {
    ...
    const downloadFile = () => {
        fetch(`${process.env.REACT_APP_API_URL}/job-seeker/generate-cv/second/${user.uuid}`, {
            method: "get", headers: {
    ...
    const fetchCV = (uuid) => {
        fetch(`${process.env.REACT_APP_API_URL}/job-seeker/generate-cv/second/${uuid}`, {
            method: "get", headers: {
                "Content-Type": "application/json", 'X-XSRF-TOKEN': decodeURIComponent(Cookie.get('XSRF-TOKEN')),
            },
        }).then(response => response.blob()).then(blob => {
...
export default CVDownloadSecond;