...
const AllListing = ({setRefresh}) => {
    ...
    useEffect(() => {
        (async () => {
            const params = helper.postJobParams(limit, pageCnt, null, filter);
    ...
    const handleClickAction = async (status, uuid) => {
        if (status && uuid) {
            await EmpPostedJobsDataService.updatePostedJob(uuid, {status: status}).then(res => {
